...
const ActionHistoryItem: FC<ActionHistoryItemProps> = ({ data }): JSX.Element => {
  ...
  return (
    <div className='flex flex-col bg-primaryContainer text-white rounded-lg  hover:cursor-pointer p-2'>
      <div
      ...
      {isExpanded && (
        <div className='flex flex-col p-2'>
          <p className="text-xs">Started: {elapsedTime(Date.parse(data.startedAt))}</p>
          <p className="text-xs">Updated: {elapsedTime(Date.parse(data.updatedAt))}</p>
          <p className="text-xs">Action: {data.key.toUpperCase()}</p>
          <p className="text-xs">Logs ID: {data.logsId}</p>
        </div>
      )}
    </div>
  );
