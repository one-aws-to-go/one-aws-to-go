...
const ActionPage = () => {
  ...
  return (
    <div className=' bg-surface h-screen overflow-y-auto'>
      <NavBar />
            ...
            extendedFork.data ? (
              <div className='flex flex-row space-x-2'>
                {extendedFork.data.actions.map((item, index) => (
                  <button disabled={mutation.isLoading} key={index} onClick={() => handleActionButtonClicked(item)} className="flex flex-col bg-primaryContainer hover:bg-primaryContainer/[.60] text-white  p-2 hover:text-primary rounded-lg">
            ...
            {actionHistory.isLoading ? (
              ...
              actionHistory.data ? (
                actionHistory.data.map((item, index) => (
                  <ActionHistoryItem key={index} data={item} />
                ))
              ) : null)}
          </div>
